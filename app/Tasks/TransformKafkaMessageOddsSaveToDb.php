...
class TransformKafkaMessageOddsSaveToDb extends Task
{
    ...
    public function __construct($subTasks, $uid, $eventOnly)
    {
    ...
    public function handle()
    {
        ...
        if (!$this->eventOnly) {
            try {
                ...
                MasterEvent::where('master_event_unique_id', $this->eventData['MasterEvent']['data']['master_event_unique_id'])
                    ->update($this->eventData['MasterEvent']['data']);

                $masterEventModel = MasterEvent::where('master_event_unique_id', $this->eventData['MasterEvent']['data']['master_event_unique_id'])
                    ->first();
            }
