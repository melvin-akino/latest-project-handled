...
class OrdersController extends Controller
{
    ...
    public function getEventMarketsDetails(string $memUID)
    {
        try {
            ...
            $eventBets = UserBet::getUserBetsByEvent($masterEvent->master_event_unique_id)->count();
            ...
            if (empty($userPlaceBet) || array_key_exists('error', $userPlaceBet) || !array_key_exists('status_code', $userPlaceBet) || $userPlaceBet->status_code != 200) {
                ...
                foreach ($split AS $row) {
                    ...
                    if ($row['place']) {
                        ...
                        $unplacedBet = WalletFacade::subtractBalance($userWalletToken, auth()->user()->uuid, trim($provCurrencyCode), ($walletAmount), "[PLACE_BET][RETURN_STAKE] - Unplaced Bet Stake: " . trim($provCurrencyCode) . " " . $row['stake']);
    ...
    public function betMatrixOrders(string $uid)
    {
        try {
            ...
            $orders   = UserBet::getUserBetsByEvent($uid)->get();
