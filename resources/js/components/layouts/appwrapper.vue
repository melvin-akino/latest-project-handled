<template>
      <div class="bg-gray-200 h-screen w-full sm:pb-0 pb-8" :class="{'flex flex-col items-center':!$store.state.isAuthenticated}">
        <nav class="flex bg-white shadow-md w-full h-16" v-if="$store.state.isAuthenticated">
              <div class="flex justify-start items-center w-full ml-16">
              ...
              <div class="flex justify-end items-center w-full mr-16">
          ...
          <img :src="logo" class="sm:w-64 w-40 sm:mt-12 mt-10" v-else>
          <slot></slot>
      </div>
</template>
